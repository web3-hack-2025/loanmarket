  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-4 overflow-auto">
        ...
        {!hideSuccessText && (
          <div className={`fixed inset-0 flex items-center justify-center transition-opacity duration-700 ${showSuccessText ? 'opacity-100' : 'opacity-0'} ${showSuccessContent ? 'opacity-0' : ''}`}>
            <h1 className="text-6xl font-bold text-white">SUCCESS</h1>
          </div>
        )}
        
        {/* Application successful content that phases in after */}
        <div className={`bg-white dark:bg-gray-800/90 rounded-lg shadow-lg p-6 md:p-8 w-full max-w-xl my-8 transition-all duration-700 ${showSuccessContent ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          ...
          <h1 className="text-3xl font-bold text-center mb-4 dark:text-white">Application Successful!</h1>
          <p className="text-gray-600 dark:text-gray-300 text-center mb-8">
            Your loan application has been successfully submitted to {selectedLoan?.provider || "Easy Crypto"}.
          </p>
          ...
          <div className="border-t border-b border-gray-200 dark:border-gray-700 py-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-gray-600 dark:text-gray-400">Loan Provider</p>
                <p className="font-medium dark:text-white">{selectedLoan?.provider ?? "Easy Crypto"}</p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Loan Product</p>
                <p className="font-medium dark:text-white">{selectedLoan?.name || "Crypto-Backed Loan"}</p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Loan Amount</p>
                <p className="font-medium dark:text-white">${requestedAmount ? Number(requestedAmount).toLocaleString('en-US', {maximumFractionDigits: 2}) : "N/A"} NZDD</p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Interest Rate</p>
                <p className="font-medium dark:text-white">{selectedLoan?.interestRate || "3.5%"}</p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Term Length</p>
                <p className="font-medium dark:text-white">{termLength ? `${termLength} months` : selectedLoan?.term || "N/A"}</p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Monthly Payment</p>
                <p className="font-medium dark:text-white">
                  ${requestedAmount && termLength ? 
                    Number(Number(requestedAmount) / Number(termLength) + Number(requestedAmount) * getInterestRateDecimal()).toLocaleString('en-US', {maximumFractionDigits: 2}) 
                    : "0.00"} NZDD
                </p>
              </div>
              <div>
                <p className="text-gray-600 dark:text-gray-400">Collateral Required</p>
                <p className="font-medium dark:text-white">{selectedLoan?.collateralRequired || "None"}</p>
              </div>
            ...
            {selectedLoan?.description && (
              <div className="mt-4">
                ...
                <p className="font-medium dark:text-white">{selectedLoan.description}</p>
              </div>
            )}
          </div>
          
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4 dark:text-white">Loan Summary</h2>
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium dark:text-white">
                    ${requestedAmount ? Number(requestedAmount).toLocaleString('en-US', {maximumFractionDigits: 2}) : "0.00"} NZDD
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-medium dark:text-white">
                    ${requestedAmount && termLength ? 
                      Number(Number(requestedAmount) * getInterestRateDecimal() * Number(termLength)).toLocaleString('en-US', {maximumFractionDigits: 2}) 
                      : "0.00"} NZDD
                  </span>
                </div>
                <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <span className="font-semibold text-blue-600 dark:text-blue-400">
                    ${requestedAmount && termLength ? 
                      Number(Number(requestedAmount) + Number(requestedAmount) * getInterestRateDecimal() * Number(termLength)).toLocaleString('en-US', {maximumFractionDigits: 2}) 
                      : "0.00"} NZDD
                  </span>
                </div>
              </div>
            ...
            <ol className="list-decimal list-inside space-y-2 text-gray-600 dark:text-gray-300">
              <li>Check your email for confirmation from {selectedLoan?.provider ?? "Easy Crypto"}</li>
              <li>Complete any additional verification steps if required</li>
              <li>Funds will be disbursed to your account within 1-3 business days</li>
            </ol>
          </div>
